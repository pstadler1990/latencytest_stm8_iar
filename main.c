    ...
    while(1) {
        ...
        case S_STATE_STORE_DATA:
            ...
            if(device_state.calibMode == C_CALIBMODE_B) {
                ...
                /* Send lower and upper calibrated thresholds to the master (for information only) */
                com_send("{\r\n");
                send_buf_value(calibValueStoredLowerThreshold);
                send_buf_value(calibValueStoredUpperThreshold);
                com_send("}\r\n");
...
init_GPIOs(void) {
  ...
  /* Debug pin, Output, PP, fast */
  GPIO_DEBUG_PORT->DDR |= (uint8_t)GPIO_DEBUG_PIN;
  GPIO_DEBUG_PORT->CR1 |= (uint8_t)GPIO_DEBUG_PIN;
  GPIO_DEBUG_PORT->CR2 |= (uint8_t)GPIO_DEBUG_PIN;
